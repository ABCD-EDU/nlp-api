...
file_path = os.path.join(current_dir, 'Heuristics', 'Negation.txt')
with open(file_path) as f:
    lines= f.readlines()
    for line in lines: 
        NEGATE.append(str(line).strip().lower()) 
...
with open(file_path) as f:
    ...
    for line in lines: 
        if len(line.split(','))<2:
            continue
        adverb, modifier = line.split(',')
        modifier_score = B_INCR if str(modifier).strip() =="B_INCR" else B_DECR
        BOOSTER_DICT[str(adverb).strip().lower()] = modifier_score
        # BOOSTER_DICT.append(str(line)) 
...
with open(file_path) as f:
    ...
    for line in lines: 
        if len(line.split(','))<2:
            continue
        idiom, modifier = line.split(',')
        SENTIMENT_LADEN_IDIOMS[str(idiom).strip().lower()] = modifier
...
class SentimentIntensityAnalyzer(object):
    ...
    def __init__(self, lexicon_file="modified_vader_lexicon.txt", emoji_lexicon="emoji_utf8_lexicon.txt"):
        # _this_module_file_path_ = os.path.abspath(getsourcefile(lambda: 0))
        # lexicon_full_filepath = os.path.join(os.path.dirname(_this_module_file_path_), lexicon_file)
        lexicon_full_filepath= os.path.join(current_dir, lexicon_file)
        with codecs.open(lexicon_full_filepath, encoding='utf-8') as f:
            self.lexicon_full_filepath = f.read()
        self.lexicon = self.make_lex_dict()

        # emoji_full_filepath = os.path.join(os.path.dirname(_this_module_file_path_), emoji_lexicon)
        emoji_full_filepath= os.path.join(current_dir, emoji_lexicon)
    ...
    def sentiment_valence(self, valence, sentitext, item, i, sentiments):
        ...
        if item_lowercase in self.lexicon:
            ...
            if (item_lowercase =="hindi" or item_lowercase == "no") and i != len(words_and_emoticons)-1 and words_and_emoticons[i + 1].lower() in self.lexicon:
